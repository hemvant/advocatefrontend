...
import { listCourtroomsByCourt, listCourts } from '../../services/courtApi';
...
export default function CourtroomList() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    listCourtroomsByCourt(courtId)
      .then(({ data: res }) => setData(res.data || []))
      .catch(() => setError('Failed to load courtrooms'))
      .finally(() => setLoading(false));
  }, [courtId]);

  return (
    <div>
      <h1 className="text-2xl font-bold text-primary mb-6">Courtrooms</h1>
      ...
      <div className="bg-white rounded-lg border border-gray-200 shadow-sm overflow-hidden">
        {!courtId ? (
          <p className="p-6 text-gray-500 text-center">Select a court to view courtrooms.</p>
        ) : loading ? (
          ...
          <p className="p-6 text-gray-500 text-center">No courtrooms for this court. Add courtrooms from the court detail page.</p>
        ) : (
          <ul className="divide-y divide-gray-200">
            {data.map((r) => (
              <li key={r.id} className="px-6 py-4 flex justify-between items-center">
                ...
                <Link to={`/courts/${r.court_id || courtId}`} className="text-sm text-primary hover:underline">View court →</Link>
              </li>
            ))}
          </ul>
        )}
